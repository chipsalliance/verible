...
# Copyright 2021-2023 The Verible Authors.
#
...
###
# Extension of the smoke test, where the output of each non-zero exitting test is 
# copied to a file where it can be later processed and looked at. The files follow a 
# log naming scheme where the exit code is first, later the name of the file and the tool
# that had a problem. The directory contains the project name to which the file belongs.
###
...
ExpectedFailCount[syntax:basejump_stl]=461
ExpectedFailCount[lint:basejump_stl]=461
ExpectedFailCount[project:basejump_stl]=567
...
function run_smoke_test() {
  ...
  for tool in $VERIBLE_TOOLS_TO_RUN ; do
	...
	file_param=${single_file}
      ...
      ${BINARY_BASE_DIR}/${tool} ${EXTRA_PARAM} ${file_param} > /tmp/${PROJECT_NAME}_${FNAME}_${TNAME} 2>&1
      local EXIT_CODE=$?
      ...
      if [ $asdf -ne 1 ]; then
        rm /tmp/${PROJECT_NAME}_${FNAME}_${TNAME}
      fi
      if [ $asdf -ne 0 ]; then
        mkdir -p /tmp/${PROJECT_NAME}-nonzeros
        mv /tmp/${PROJECT_NAME}_${FNAME}_${TNAME} /tmp/${PROJECT_NAME}-nonzeros/
        mv /tmp/${PROJECT_NAME}-nonzeros/${PROJECT_NAME}_${FNAME}_${TNAME} /tmp/${PROJECT_NAME}-nonzeros/${EXIT_CODE}-${FNAME}_${TNAME}
      fi
