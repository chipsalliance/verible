...
absl::string_view AlwaysFFNonBlockingRule::Name() { return "always-ff-non-blocking"; }
...
std::string AlwaysFFNonBlockingRule::GetDescription(DescriptionType description_type) {
...
void AlwaysFFNonBlockingRule::HandleSymbol(const verible::Symbol& symbol,
  ...
  if (always_ff_matcher_.Matches(symbol, &manager)) {
    for (const auto& match : SearchSyntaxTree(symbol, NodekAssignmentStatement())) {
      auto *node = dynamic_cast<const verible::SyntaxTreeNode *>(match.match);

      if (node == nullptr)
        continue ;

      auto *leaf = dynamic_cast<const verible::SyntaxTreeLeaf *>(node->children()[1].get());
