...
const char ForbidDefparamRule::kMessage[] =
    "Do not use defparam.";

std::string ForbidDefparamRule::GetDescription(
    DescriptionType description_type) {
  return absl::StrCat(
      "Do not use defparam. See:",
...
void ForbidDefparamRule::HandleSymbol(
  ...
  if (matcher_.Matches(symbol, &manager)) {
    const auto& defparam_token = GetSubtreeAsLeaf(
            symbol, NodeEnum::kParameterOverride, 0).get();
    CHECK_EQ(defparam_token.token_enum, TK_defparam);
    violations_.push_back(verible::LintViolation(
            defparam_token, kMessage, context));
  }
}
