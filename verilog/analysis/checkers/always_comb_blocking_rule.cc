...
absl::string_view AlwaysCombBlockingRule::Name() { return "always-comb-blocking"; }
...
std::string AlwaysCombBlockingRule::GetDescription(DescriptionType description_type) {
...
void AlwaysCombBlockingRule::HandleSymbol(const verible::Symbol& symbol,
  ...
  if (always_comb_matcher_.Matches(symbol, &manager)) {
    for (const auto& match : SearchSyntaxTree(symbol, NodekNonblockingAssignmentStatement())) {
      auto *node = dynamic_cast<const verible::SyntaxTreeNode *>(match.match);

      if (node == nullptr)
        continue ;

      auto leaf = verible::GetSubtreeAsLeaf(*node, NodeEnum::kNonblockingAssignmentStatement, 1);
