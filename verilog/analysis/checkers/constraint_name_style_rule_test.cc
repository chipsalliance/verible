...
TEST(ConstraintNameStyleRuleTest, RejectTests) {
  ...
  const std::initializer_list<LintTestCase> kTestCases = {
      {"class foo; rand logic a; constraint ",
       {kToken, "_c"}, " { a == 16; } endclass"},
      {"class foo; rand logic a; constraint ",
       {kToken, "no_suffix"}, " { a == 16; } endclass"},

      {"class foo; rand logic a; constraint ",
       {kToken, "WrongName"}, " { a == 16; } endclass"},
      {"class foo; rand logic a; constraint ",
       {kToken, "WrongName_c"}, " { a == 16; } endclass"},

      {"class foo; rand logic a; constraint ",
       {kToken, "wrong_name_C"}, " { a == 16; } endclass"},

      {"class foo; rand logic a; constraint ",
       {kToken, "WRONG_NAME"}, " { a == 16; } endclass"},

      {"class foo; rand logic a; constraint ",
       {kToken, "WRONG_NAME_c"}, " { a == 16; } endclass"},

      {"class foo; rand logic a; constraint ",
       {kToken, "WRONG_c"}, " { a == 16; } endclass"},

      {"class foo; rand logic a, b; "
       "constraint ", {kToken, "FIRST_C"}, " { a == 16; }; "
       "constraint ", {kToken, "SECOND_C"}, " {b == 20; } endclass"},

      {"class foo; rand logic a; "
       "constraint ", {kToken, "FIRST_C"}, " { a == 16; } endclass;"
