...
TEST(UvmMacroSemicolonRule, AcceptedUvmMacroCallTests) {
  const std::initializer_list<LintTestCase> kTestCases = {
    ...
    {"module top;"
       "initial begin\n"
         "`uvm_config_db#(virtual top_if)::set(\"uvm_test_top\", \" \",  \"if\", if);"
        "end\n"
     "endmodule\n"},
...
TEST(UvmMacroSemicolonRule, WrongUvmMacroTest) {
  ...
  const std::initializer_list<LintTestCase> kTestCases = {
    // File/Package scope
    {"`uvm_analysis_imp_decl(_RX)", {kToken,";"},
     "`uvm_analysis_imp_decl(_TX)", {kToken,";"}},
    {"`uvm_info_begin(\"msg_id\",\"message\", UVM_LOW)", {kToken,";"},
       "`uvm_message_add_tag(\"my_color\", \"red\")" , {kToken,";"},
       "`uvm_message_add_int(my_int, UVM_DEC)" , {kToken,";"},
     "`uvm_info_end", {kToken,";"}},

    // Function/Task scope
    {"function void f();"
      "`uvm_info(\"msg_id\",\"message\", UVM_LOW)", {kToken,";"}, 
     "endfunction\n"},
    {"function void f();"
      "`uvm_info_begin(\"msg_id\",\"message\", UVM_LOW)", {kToken,";"},
        "`uvm_message_add_tag(\"my_color\", \"red\")" , {kToken,";"},
    ...
    {"function void f();"
        "`uvm_info(\"msg_id\",\"message\", UVM_LOW)", {kToken,";"}, 
        "`uvm_warning(\"msg_id\",\"message\")", {kToken,";"}, 
        "`uvm_error(\"msg_id\",\"message\")", {kToken,";"}, 
        "`uvm_fatal(\"msg_id\",\"message\")", {kToken,";"},
    "endfunction\n"},
    {"task t();"
      "`uvm_info(\"msg_id\",\"message\", UVM_LOW)", {kToken,";"}, 
     "endtask\n"},
    ...
    // Class scope
    {"class c extends uvm_object;"
      "`uvm_object_utils(c)", {kToken,";"},
    "endclass\n"},
    {"class s extends uvm_sequence;"
      "`uvm_declare_p_sequencer(SEQUENCER)", {kToken,";"},
    "endclass\n"},
    {"class c extends uvm_object;",
      "`uvm_object_utils_begin(c)", {kToken,";"}, "\n" 
      "`uvm_object_utils_end\n",
     "endclass\n"},
    {"class c extends uvm_object;"
      "function void f();"
    ...
    {"task t();"
      "for(int i=0;i<10;i++)"
        "`uvm_info(\"msg_id\",\"message\", UVM_LOW)", {kToken,";"},
      "for(int i=0;i<10;i++)"
        "`uvm_warning(\"msg_id\",\"message\", UVM_LOW)", {kToken,";"}, 
      "for(int i=0;i<10;i++)"
        "`uvm_error(\"msg_id\",\"message\", UVM_LOW)", {kToken,";"},
      "for(int i=0;i<10;i++)"
        "`uvm_fatal(\"msg_id\",\"message\", UVM_LOW)", {kToken,";"}, 
     "endtask\n"},
    {"task t();"
      "for(int i=0;i<10;i++) begin"
        "`uvm_info(\"msg_id\",\"message\", UVM_LOW)", {kToken,";"},
