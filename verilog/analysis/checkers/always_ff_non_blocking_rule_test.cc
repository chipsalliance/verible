...
TEST(AlwaysFFNonBlockingRule, FunctionFailures) {
  ...
  const std::initializer_list<LintTestCase> kAlwaysFFNonBlockingTestCases = {
      ...
      {"module m;\nalways_ff @(posedge c) begin a ", {kToken, "="}, " b; end\nendmodule"},
      {"module m;\nalways_ff @(posedge c)  begin if (sel == 0) a ", {kToken, "="}, " b; "
          "else a ", {kToken, "="}, " 1'b0; end\nendmodule"},
  ...
  RunLintTestCases<VerilogAnalyzer, AlwaysFFNonBlockingRule>(kAlwaysFFNonBlockingTestCases);
}
