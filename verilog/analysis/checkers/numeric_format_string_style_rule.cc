...
absl::string_view NumericFormatStringStyleRule::Name() { return "numeric-format-string-style"; }
...
std::string NumericFormatStringStyleRule::GetDescription(
  ...
  return absl::StrCat("Checks that string literals with numeric format specifiers "
    "have proper prefixes for hex and bin values and no prefixes for decimal values.");
}
...
void NumericFormatStringStyleRule::CheckAndReportViolation(
  ...
  if (pos >= 2 && (text[pos-2] == '0' || text[pos-2] == '\'')) {
    ...
    if (std::none_of(prefixes.begin(), prefixes.end(),
      ...
      violations_.insert(LintViolation(TokenInfo(token.token_enum(),
                                                 text.substr(pos - 2, len + 2)),
                                       kMessage));
    }
  } else {
    // Report just radix
...
void NumericFormatStringStyleRule::HandleToken(const TokenInfo& token) {
  ...
  // TODO(fangism): Refactor this. Right now it's a copy-paste from macro_name_style_rule.cc
  ...
  if (token_enum != TK_StringLiteral)
    return ;
  ...
  for (size_t pos = 0 ; pos < text.size() ;) {
    ...
    for (size_t itr = pos + 1 ; itr < text.size() ; ++itr) {
      ...
      if (absl::ascii_isdigit(text[itr]))
        continue ;
