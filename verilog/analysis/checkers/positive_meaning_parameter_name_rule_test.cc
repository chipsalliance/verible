...
TEST(PositiveMeaningParameterNameRuleTest, AcceptTests) {
  ...
  RunLintTestCases<VerilogAnalyzer, PositiveMeaningParameterNameRule>(kTestCases);
}
...
TEST(PositiveMeaningParameterNameRuleTest, RejectTests) {
  ...
  const std::initializer_list<LintTestCase> kTestCases = {
      {"module foo; parameter int ",
       {kToken, "disable_foo"}, " = 1; endmodule"},
      {"module foo; parameter int ",
       {kToken, "DisableFoo"}, " = 1; endmodule"},
      {"module foo; parameter int ",
       {kToken, "DiSaBle_Foo"}, " = 1; endmodule"},

      {"module foo; localparam ",
       {kToken, "disable_bar"}, " = 1; endmodule"},
      {"module foo; localparam ",
       {kToken, "DisableBar"}, " = 1; endmodule"},
      {"module foo; localparam ",
       {kToken, "dIsaBleBar"}, " = 1; endmodule"},

      {"module foo #(parameter int ",
       {kToken, "disable_hello"}, " = 1); endmodule"},
      {"module foo #(parameter int ",
       {kToken, "disableHello"}, " = 1); endmodule"},
      {"module foo #(parameter int ",
       {kToken, "DiSable_hello"}, " = 1); endmodule"},
      {"module foo #(parameter int ",
       {kToken, "disable_hello"}, "); endmodule"},
      {"module foo #(parameter int ",
       {kToken, "disableHello"}, "); endmodule"},
      {"module foo #(parameter int ",
       {kToken, "DiSable_hello"}, "); endmodule"},

      {"module foo #(parameter int enable_f = 1, ",
       {kToken, "disable_s"}, " = 1); endmodule"},

      {"class foo #(parameter int ",
       {kToken, "disable_opt"}, " = 1); endclass"},
      {"class foo #(parameter int ",
       {kToken, "disableOpt"}, " = 1); endclass"},
      {"class foo #(parameter int ",
       {kToken, "DiSABle_Opt"}, " = 1); endclass"},
      {"class foo #(parameter int ",
       {kToken, "disable_opt"}, "); endclass"},
      {"class foo #(parameter int ",
       {kToken, "disableOpt"}, "); endclass"},
      {"class foo #(parameter int ",
       {kToken, "DiSABle_Opt"}, "); endclass"},

      {"package foo; parameter int ",
       {kToken, "disable_sth"}, " = 1; endpackage"},

      {"module foo #(parameter int ",
       {kToken, "DiSable_hello"}, "); "
       " parameter int ", {kToken, "disableAbC"}, " = 1;"
       "endmodule"},
