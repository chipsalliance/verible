...
class AutoExpander {
 public:
  ...
  class Module {
   public:
    ...
    void EmitNonAnsiPortList(
        std::ostream &output, size_t indent, absl::string_view heading,
    ...
    void EmitPortConnections(std::ostream &output, size_t indent,
    ...
    void EmitPortDeclarations(
        std::ostream &output, size_t indent, PortDeclStyle style,
        const std::function<bool(const Port &)> &pred) const;
    ...
    void EmitUndeclaredOutputWireDeclarations(
        std::ostream &output, size_t indent, absl::string_view auto_span) const;
  ...
  std::optional<Expansion> ExpandAutoDeclarations(
      ...
      absl::string_view comment_span, size_t indent,
 ...
 private:
  ...
  // Format style to use in AUTO expansions
  FormatStyle format_style_;
...
void AutoExpander::Module::EmitNonAnsiPortList(
    std::ostream &output, const size_t indent, const absl::string_view heading,
    const std::function<bool(const Port &)> &pred) const {
  bool first = true;
...
void AutoExpander::Module::EmitPortConnections(
    std::ostream &output, const size_t indent, const absl::string_view header,
  ...
  GenerateConnections(tmpl, [&](const Port &port, const Connection &connected) {
    ...
    if (first) {
      if (output.tellp() != 0) output << ',';
      output << '\n' << Spacer(indent) << "// " << header;
      first = false;
    } else {
      output << ',';
    }
    output << '\n'
           << Spacer(indent) << '.' << port.name << '(' << connected.port_name;
    ...
    if (port.packed_dimensions.size() > 1 ||
      ...
      output << "  /*";  // Indent the comment,
                         // as the formatter will do it anyway
...
void AutoExpander::Module::EmitUndeclaredOutputWireDeclarations(
    std::ostream &output, const size_t indent,
...
std::optional<AutoExpander::Expansion> AutoExpander::ExpandAutoarg(
  ...
  module.EmitNonAnsiPortList(
      new_text, format_style_.PortDeclarationsIndentation(), "Inputs",
      [&](const Port &port) {
        return port.direction == Port::Direction::kInput &&
               !predeclared_ports.contains(port.name);
      });
  module.EmitNonAnsiPortList(
      new_text, format_style_.PortDeclarationsIndentation(), "Inouts",
      [&](const Port &port) {
        return port.direction == Port::Direction::kInout &&
               !predeclared_ports.contains(port.name);
      });
  module.EmitNonAnsiPortList(
      new_text, format_style_.PortDeclarationsIndentation(), "Outputs",
      [&](const Port &port) {
        return port.direction == Port::Direction::kOutput &&
               !predeclared_ports.contains(port.name);
      });
  if (new_text.tellp() != 0) new_text << '\n';
...
std::optional<AutoExpander::Expansion> AutoExpander::ExpandAutoinst(
  ...
  std::ostringstream new_text;
  size_t connection_indent =
      format_style_.indentation_spaces + format_style_.NamedPortIndentation();
  inst_module.EmitPortConnections(
      new_text, connection_indent, "Inputs",
      [&](const Port &port) {
        return port.direction == Port::Direction::kInput &&
               port.declaration != Port::Declaration::kUndeclared &&
      ...
      tmpl);
  inst_module.EmitPortConnections(
      new_text, connection_indent, "Inouts",
      [&](const Port &port) {
        return port.direction == Port::Direction::kInout &&
               port.declaration != Port::Declaration::kUndeclared &&
               !preconnected_ports.contains(port.name);
      ...
      tmpl);
  inst_module.EmitPortConnections(
      new_text, connection_indent, "Outputs",
      [&](const Port &port) {
        return port.direction == Port::Direction::kOutput &&
               port.declaration != Port::Declaration::kUndeclared &&
               !preconnected_ports.contains(port.name);
      ...
      tmpl);
  if (new_text.tellp()) {
    new_text << '\n' << Spacer(format_style_.indentation_spaces);
  }
...
std::optional<AutoExpander::Expansion> AutoExpander::ExpandAutoPorts(
  ...
  const size_t indent = in_header ? format_style_.PortDeclarationsIndentation()
                                  : format_style_.indentation_spaces;
  ...
  auto result = ExpandAutoDeclarations(
      *module, auto_span, comment_span,  // Matched AUTO comment
      indent, description,
      [direction, style, indent](const Module &module, std::ostream &output) {
        module.EmitPortDeclarations(
            output, indent, style, [direction](const Port &port) {
...
std::optional<AutoExpander::Expansion> AutoExpander::ExpandAutowire(
  ...
  return ExpandAutoDeclarations(
      ...
      [this, auto_span](const Module &module, std::ostream &output) {
        module.EmitUndeclaredOutputWireDeclarations(
            output, format_style_.indentation_spaces, auto_span);
      });
}
...
std::optional<AutoExpander::Expansion> AutoExpander::ExpandAutoreg(
  ...
  return ExpandAutoDeclarations(
      ...
      [this, auto_span](const Module &module, std::ostream &output) {
        module.EmitUnconnectedOutputRegDeclarations(
            output, format_style_.indentation_spaces, auto_span);
      });
}
...
// Converts AutoExpander::Expansion structs to LSP TextEdits
std::vector<TextEdit> ConvertAutoExpansionsToTextEdits(
    const TextStructureView &text_structure,
    const std::vector<AutoExpander::Expansion> &expansions) {
  std::vector<TextEdit> edits;
  edits.reserve(expansions.size());
...
std::vector<CodeAction> GenerateAutoExpandCodeActions(
  ...
  result.emplace_back(CodeAction{
      ...
      .edit = {.changes = {{p.textDocument.uri,
                            ConvertAutoExpansionsToTextEdits(
