...
class AutoExpander {
 public:
  ...
  std::optional<TextEdit> ExpandAutoDeclarations(
      Module &module, const std::regex &re, absl::string_view description,
      const std::function<void(const Module &, std::ostream &)> &emit) const;

  // Expands AUTOINPUT for the given module
  std::optional<TextEdit> ExpandAutoinput(Module &module) const;

  // Expands AUTOINOUT for the given module
  std::optional<TextEdit> ExpandAutoinout(Module &module) const;
...
std::optional<TextEdit> AutoExpander::ExpandAutoDeclarations(
    Module &module, const std::regex &re, const absl::string_view description,
    const std::function<void(const Module &, std::ostream &)> &emit) const {
  ...
  const LineColumnRange range =
      text_structure_.GetRangeForText({module_span.begin() + match.position(),
                                       static_cast<size_t>(match.length())});
  std::stringstream new_text;
  new_text << match.str(1);  // Matched AUTO comment
  new_text << indent_ << "// Beginning of automatic " << description << '\n';
  if (match.length(2)) new_text << match.str(2);  // Existing declarations
...
std::optional<TextEdit> AutoExpander::ExpandAutoinput(Module &module) const {
  auto result = ExpandAutoDeclarations(
      module, autoinput_re_, "inputs (from autoinst inputs)",
      [this](const Module &module, std::ostream &output) {
        module.EmitUndeclaredInputDeclarations(output, indent_);
      });
  if (result) {
    const int offset =
        text_structure_.GetLineColumnMap().OffsetAtLine(result->range.end.line);
    module.MarkUndeclaredPortsAsAutogenerated(Port::Direction::Input, offset);
  }
  return result;
}

std::optional<TextEdit> AutoExpander::ExpandAutoinout(Module &module) const {
  auto result = ExpandAutoDeclarations(
      module, autoinout_re_, "inouts (from autoinst inouts)",
      [this](const Module &module, std::ostream &output) {
  ...
  if (result) {
    const int offset =
        text_structure_.GetLineColumnMap().OffsetAtLine(result->range.end.line);
    module.MarkUndeclaredPortsAsAutogenerated(Port::Direction::Inout, offset);
  }
  return result;
}

std::optional<TextEdit> AutoExpander::ExpandAutooutput(Module &module) const {
  auto result = ExpandAutoDeclarations(
      module, autooutput_re_, "outputs (from autoinst outputs)",
      [this](const Module &module, std::ostream &output) {
  ...
  if (result) {
    const int offset =
        text_structure_.GetLineColumnMap().OffsetAtLine(result->range.end.line);
    module.MarkUndeclaredPortsAsAutogenerated(Port::Direction::Output, offset);
  }
  return result;
}
...
std::vector<TextEdit> AutoExpander::Expand() {
  ...
  for (Module *const module : buffer_modules) {
    ...
    // AUTO port declarations
    if (const auto edit = ExpandAutoinput(*module)) {
      edits.push_back(*edit);
    }
    if (const auto edit = ExpandAutoinout(*module)) {
      edits.push_back(*edit);
    }
