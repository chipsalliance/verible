...
#include "common/util/tree-operations.h"
#include "gtest/gtest.h"
#include "verilog/analysis/verilog-project.h"
#include "verilog/tools/kythe/indexing-facts-tree.h"
#include "verilog/tools/kythe/verilog-extractor-indexing-fact-type.h"
 ...
 public:
  ...
  T Extract() {
    return ExtractFiles(
        ...
        /* project= */ this,
        /* file_names= */
        {std::string(verible::file::Basename(OnlyFileName()))}, &errors_);
  }
...
class IncludeTestProject : protected SimpleTestProject {
 public:
  ...
  using SimpleTestProject::Errors;
  ...
  const IndexingFactNode expected(  //
      ...
      T(project.XUnit().ExpectedFileData(),
        ...
        T(D{
            ...
            Anchor(kTestCase.expected_tokens[1], kTestCase.code),
            Anchor(project.IncludeFile().source_file->ResolvedPath()),
        }),
...
TEST(FactsTreeExtractor, IncludedFileContainsParseError) {
  ...
  const auto statuses(project.Errors());
