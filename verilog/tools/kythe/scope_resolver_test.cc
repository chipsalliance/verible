...
TEST(FlattenedScopeTests, SearchForDefinition) {
  ...
  const VName* vname =
      flattened_scope_resolver.SearchForVNameInScope(signatures[0], names[1]);
  EXPECT_EQ(vname->signature, signatures[1]);

  const VName* vname2 =
      flattened_scope_resolver.SearchForVNameInScope(signatures[0], names[2]);
  EXPECT_EQ(vname2->signature, signatures[2]);

  const VName* vname3 =
      flattened_scope_resolver.SearchForVNameInScope(signatures[0], names[3]);
  EXPECT_EQ(vname3, nullptr);

  const VName* vname4 =
      flattened_scope_resolver.SearchForVNameInScope(signatures[2], names[1]);
...
TEST(FlattenedScopeTests, SearchForNestedDefinition) {
  ...
  const VName* vname0 =
      flattened_scope_resolver.SearchForVNameInScope(signatures[0], names[1]);
  const VName* vname1 =
      flattened_scope_resolver.SearchForVNameInScope(signatures[1], names[3]);
  const VName* vname2 =
      flattened_scope_resolver.SearchForVNameInScope(signatures[5], names[3]);
  const VName* vname3 =
      flattened_scope_resolver.SearchForVNameInScope(signatures[5], names[6]);
...
TEST(VerticalScopeTests, SearchForDefinition) {
  ...
  {
    Scope scope;
    VerticalScopeResolver::AutoPop p1(&vertical_scope_resolver, &scope);
...
TEST(VerticalScopeTests, NestedScopeSearch) {
  VerticalScopeResolver vertical_resolver;

  EXPECT_TRUE(vertical_resolver.empty());
  Scope scope;
  VerticalScopeResolver::AutoPop p1(&vertical_resolver, &scope);
  {
