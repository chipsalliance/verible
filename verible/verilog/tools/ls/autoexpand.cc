...
class AutoExpander {
 public:
  ...
  struct Match {
    std::string_view auto_span;     // Span of the entire AUTO
    std::string_view comment_span;  // Span of the AUTO pragma comment
  };

  // A single AUTO expansion in terms of the replaced span and expanded text
  struct Expansion {
    std::string_view replaced_span;  // Span that is to be replaced
  ...
  struct ConnectedInstance {
    std::optional<std::string_view> instance;  // Name of the instance a port
                                               // is connected to
    std::string_view type;  // Type of the instance a port is connected to
  };
  ...
  using Dimension = std::variant<std::string_view, size_t, DimensionRange>;
  ...
  struct Port final : Net {
    ...
    std::string_view::const_iterator it;  // Location of the port's declaration
                                          // in the source file
  ...
  struct Template {
    using Map = absl::flat_hash_map<std::string_view, std::vector<Template>>;

    std::string_view::const_iterator it;    // Location of the template in the
                                            // source file
    ...
    absl::flat_hash_map<std::string_view, Connection>
  ...
  class Module {
   public:
    ...
    void EmitUndeclaredWireDeclarations(std::ostream &output,
                                        std::string_view auto_span) const;
    ...
    void GenerateConnections(
        std::string_view instance_name, const Template *tmpl,
    ...
    void RetrieveDependencies(
        const absl::node_hash_map<std::string_view, Module> &modules);

    // Retrieves the matching template from a typename -> template map
    const Template *GetAutoTemplate(
        std::string_view type_id, std::string_view instance_name,
        std::string_view::const_iterator instance_it) const;
  ...
  absl::flat_hash_set<std::string_view> GetPortsListedBefore(
      const Symbol &module, std::string_view::const_iterator it) const;

  // Retrieves port names from a module instance connected before the given
  // location
  absl::flat_hash_set<std::string_view> GetPortsConnectedBefore(
      const Symbol &instance, std::string_view::const_iterator it) const;
  ...
  std::optional<Expansion> ExpandAutoDeclarations(
      const Module &module, Match match, std::string_view description,
 ...
 private:
  ...
  // AUTOINST)
  std::optional<std::string_view> FindSpanToReplace(
      const Symbol &symbol, std::string_view auto_span) const;
  ...
  absl::node_hash_map<std::string_view, Module> modules_;
...
void AutoExpander::Port::EmitConnectionComment(std::ostream &output) const {
  ...
  if (!instance) return;
  const std::string_view type = conn_inst[0].type;
  switch (direction) {
...
void AutoExpander::Module::EmitNonAnsiPortList(
    std::ostream &output, const std::string_view heading,
    const std::function<bool(const Port &)> &pred) const {
  bool first = true;
...
void AutoExpander::Module::EmitPortConnections(
    std::ostream &output, const std::string_view instance_name,
...
void AutoExpander::Module::EmitUnconnectedOutputRegDeclarations(
    std::ostream &output, const std::string_view auto_span) const {
  absl::flat_hash_set<std::string_view> declared_regs;
  for (const auto &reg : FindAllRegisterVariables(symbol_)) {
    ...
    const std::string_view reg_name = reg_name_leaf->get().text();
...
void AutoExpander::Module::RetrieveAutoTemplates() {
  std::string_view autotmpl_search_span = StringSpanOfSymbol(symbol_);
  std::string_view autotmpl_span;
  std::string_view autotmpl_inst_name;
  while (RE2::FindAndConsume(&autotmpl_search_span, *autotemplate_re_,
    ...
    std::string_view autotmpl_conn_search_span = autotmpl_span;
    std::string_view instance_port_name;
    std::string_view module_port_name;
    ...
    std::string_view autotmpl_type_search_span = autotmpl_span;
    std::string_view instance_type_name;
...
const AutoExpander::Template *AutoExpander::Module::GetAutoTemplate(
    const std::string_view type_id, const std::string_view instance_name,
    const std::string_view::const_iterator instance_it) const {
...
std::vector<AutoExpander::Dimension> GetDimensionsFromNodes(
  ...
  for (auto &dimension : dimension_nodes) {
    for (const auto &scalar :
      ...
      const std::string_view span = StringSpanOfSymbol(scalar_value);
...
void AutoExpander::Module::PutDeclaredPort(const SyntaxTreeNode &port_node) {
  ...
  const std::string_view dir_span = dir_leaf->get().text();
...
absl::flat_hash_set<std::string_view> AutoExpander::GetPortsListedBefore(
    const Symbol &module, const std::string_view::const_iterator it) const {
  absl::flat_hash_set<std::string_view> ports_before;
...
absl::flat_hash_set<std::string_view> AutoExpander::GetPortsConnectedBefore(
    const Symbol &instance, const std::string_view::const_iterator it) const {
  absl::flat_hash_set<std::string_view> ports_before;
...
std::optional<std::string_view> FindSpanInSymbol(const Symbol &symbol,
                                                 const RE2 &re) {
  const std::string_view symbol_span = StringSpanOfSymbol(symbol);
  std::string_view match;
...
std::optional<AutoExpander::Expansion> AutoExpander::ExpandAutoinst(
    Module *module, const Symbol &instance, std::string_view type_id) {
...
std::optional<AutoExpander::Expansion> AutoExpander::ExpandAutoDeclarations(
    const Module &module, const Match match, const std::string_view description,
    const std::function<void(const Module &, std::ostream &)> &emit) const {
...
std::optional<AutoExpander::Expansion> AutoExpander::ExpandAutoPorts(
  ...
  const std::string_view module_span = StringSpanOfSymbol(module->Symbol());
  ...
  const std::string_view description = direction == Port::Direction::kInput
...
absl::flat_hash_set<AutoKind> AutoExpander::FindAutoKinds() {
  ...
  std::string_view search_span = expand_span_;
  std::string_view auto_str;
...
std::optional<std::string_view> AutoExpander::FindSpanToReplace(
    const Symbol &symbol, const std::string_view auto_span) const {
  const std::string_view symbol_span = StringSpanOfSymbol(symbol);
