...
std::string NLCountAndfirstWord(absl::string_view str) {
  std::string result;
  int newline_count = 0;
  absl::string_view::iterator begin = str.begin();
  ...
  absl::string_view::iterator end_str = begin;
...
std::string lastWordAndNLCount(absl::string_view str) {
  std::string result;
  int newline_count = 0;
  absl::string_view::iterator back = str.end() - 1;
  for (/**/; back >= str.begin(); --back) {
    if (!isspace(*back)) break;
    newline_count += (*back == '\n');
  }

  absl::string_view::iterator start_str = back;
