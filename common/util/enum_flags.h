...
Usage: (in your .cc file)

namespace your_namespace {

// enum class MyEnumType { ... }
// is defined in this namespace, in a header file.

// This mapping defines how this enum is displayed and parsed.
// By providing this mapping, the other necessary flag functions are generated
// for you.
...
std::ostream& operator<<(std::ostream& stream, MyEnumType p) {
  static const auto* flag_map = MakeEnumToStringMap(kMyEnumTypeStringMap);
  return stream << flag_map->find(p)->second;
}

bool AbslParseFlag(absl::string_view text, MyEnumType* mode,
                   std::string* error) {
  static const auto* flag_map = MakeStringToEnumMap(kMyEnumTypeStringMap);
