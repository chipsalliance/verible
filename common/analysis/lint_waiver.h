...
class LintWaiver {
 ...
 public:
  ...
  void WaiveWithRegex(std::string_view rule_name, const std::string &regex);
 ...
 private:
  ...
  // and waiver_re_map_.
  std::map<std::string_view, LineNumberSet> waiver_map_;
  std::map<std::string_view, RegexVector> waiver_re_map_;
