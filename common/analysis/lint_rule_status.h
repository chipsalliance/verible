...
class LintStatusFormatter {
 public:
  ...
  // line printed when enabled. The violations contained in the statuses are
  // sorted by their occurrence in the code and are not grouped by the status
  // object. Path is the file path of original file. This is needed because it
  // is not contained in status. Base is the string_view of the entire contents,
  // used only for byte offset calculation.
